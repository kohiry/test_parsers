...
def download_all_photos(folder_path: str):
    queue1 = Queue()
    queue2 = Queue()
    p1 = Thread(target=MultiprocParser().get_albums, args=(queue1,))
    p2 = Thread(target=MultiprocParser().get_photo, args=(queue2, queue1))
    p3 = Thread(target=SavePhotoAlbum(folder_path).save, args=(queue2,))
    p1.start()
    p2.start()
    p3.start()
    p1.join()
